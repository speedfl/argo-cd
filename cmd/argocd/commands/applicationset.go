...
func showAppSetTemplate(metadata arogappsetv1.ApplicationSetTemplateMeta, template map[string]interface{}) {
	fmt.Println("Template:")
	fmt.Println("Metadata:")
	errMeta := PrintResource(metadata, "yaml")
	errors.CheckError(errMeta)
	fmt.Println("Spec:")
	errSpec := PrintResource(template, "yaml")
	errors.CheckError(errSpec)
}
...
func printApplicationSetTable(apps []arogappsetv1.ApplicationSet, output *string) {
	...
	for _, app := range apps {
		...
		var template map[string]interface{}
		err := json.Unmarshal(app.Spec.Template.Spec.Raw, &template)
		errors.CheckError(err)

		project, ok := template["project"].(string)
		...
		if *output == "wide" {
			...
			source, ok := template["source"].(map[string]interface{})
...
func printAppSetSummaryTable(appSet *arogappsetv1.ApplicationSet, template map[string]interface{}) {
	...
	destination, ok := template["destination"].(map[string]interface{})
	...
	if syncPolicyModel, ok := template["syncPolicy"].(map[string]interface{}); ok {
